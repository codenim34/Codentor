...
export async function GET(request) {
  try {
    ...
    const interviewSessions = await InterviewSession.find({ userId, status: 'ended' })
      .sort({ createdAt: -1 })
      .limit(5);
    ...
    const averageScore = interviewSessions.length > 0
      ? Math.round(interviewSessions.reduce((sum, s) => sum + (s.aiSummary?.score || 0), 0) / interviewSessions.length)
      : 0;

    const allWeaknesses = interviewSessions.flatMap(s => s.aiSummary?.weaknesses || []);
    const allStrengths = interviewSessions.flatMap(s => s.aiSummary?.strengths || []);
...
${interviewSummary.map(s => `- ${s.level} ${s.role}: Score ${s.score}/100`).join('\n')}
...
Provide 3-5 actionable suggestions. Be specific and encouraging.`;
    ...
    try {
      ...
      aiSuggestions = {
        ...
        nextSteps: ["Take more interviews", "Review your weaknesses", "Practice technical concepts"],
        motivationalMessage: "You're on the right track! Keep learning and growing."
      };
    }
    ...
    return NextResponse.json({ 
      ...
      data: {
        suggestions: [{
          ...
          description: "Take your first AI interview to get personalized feedback!",
          priority: "high",
          category: "preparation"
        }],
        nextSteps: ["Take an AI interview", "Explore learning roadmaps", "Practice coding"],
