 ...
 */
async function sendTaskReminderEmail(userEmail, userName, task, reminderType) {
...
async function sendTestEmail(userEmail, userName) {
  console.log('sendTestEmail called for:', userEmail);
  
  const transporter = createTransporter();
  
  if (!transporter) {
    console.error('No transporter created - email service not configured');
    return { 
      success: false, 
      message: 'Email service not configured. Please add SMTP credentials to .env.local file.' 
    };
  }
  ...
  try {
    ...
    return { 
      success: false, 
      message: `Failed to send email: ${error.message}. Check server logs for details.` 
    };
  }
}
